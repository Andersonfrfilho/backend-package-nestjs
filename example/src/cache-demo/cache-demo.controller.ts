import { Controller, Get, Post, Delete, Query, Body, Inject } from '@nestjs/common';
...
export class CacheDemoController {
  ...
  async set(
    ...
    const logContext = { className: CacheDemoController.name, methodName: 'set' };
    ...
    await this.withCtx(logContext, () => this.cache.set(key, value, ttlSeconds));
  ...
  async get(@Query('key') key: string) {
    const logContext = { className: CacheDemoController.name, methodName: 'get' };
    ...
    const value = await this.withCtx(logContext, () => this.cache.get<unknown>(key));
  ...
  async del(@Query('key') key: string) {
    const logContext = { className: CacheDemoController.name, methodName: 'del' };
  ...
  @Post('set-encrypted')
  async setEncrypted(
    @Body() body: { key: string; value: unknown; ttl?: number; secret?: string },
  ) {
    const logContext = { className: CacheDemoController.name, methodName: 'setEncrypted' };
    ...
    await this.withCtx(logContext, () => this.cache.setEncrypted(key, value, ttl, secret));
   ...
   */
  @Get('get-encrypted')
  async getEncrypted(@Query('key') key: string, @Query('secret') secret?: string) {
    const logContext = { className: CacheDemoController.name, methodName: 'getEncrypted' };
  ...
  @Get('demo')
  async demo() {
    const logContext = { className: CacheDemoController.name, methodName: 'demo' };
  ...
  @Get('demo-encrypted')
  async demoEncrypted() {
    const logContext = { className: CacheDemoController.name, methodName: 'demoEncrypted' };
    const key = 'demo:encrypted-key';
    const value = { secret: 'sensitive data', ts: Date.now() };

    return this.withCtx(logContext, async () => {
      await this.cache.setEncrypted(key, value, 60);

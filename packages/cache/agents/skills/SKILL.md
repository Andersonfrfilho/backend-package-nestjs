...
export class MyService {
  ...
  async getCachedData() {
    const cached = await this.cache.get({ key: "my-key" });
    if (cached) return cached;

    const data = await fetchData();
    await this.cache.set({ key: "my-key", value: data, ttlInSeconds: 60 }); // 1 min TTL
    return data;
  }
}
```
...
- **Type naming**: tipos com sufixo `Params`/`Result` devem seguir **PascalCase** (ex.: `GetParams`, `SetEncryptedParams`).
